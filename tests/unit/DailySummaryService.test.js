 ...
 */
const DailySummaryService_1 = require("../../src/modules/reports/DailySummaryService");
...
describe('DailySummaryService', () => {
    ...
    let userId;
    beforeEach(async () => {
        service = new DailySummaryService_1.DailySummaryService(setup_1.testPrisma, mockWhatsAppProvider);
        // Create test user
        const user = await setup_1.testPrisma.user.create({
            data: {
                phone: '+1234567890',
                preferences: {
                    create: {
    ...
    describe('composeDailySummary', () => {
        ...
        test('should calculate meal statistics correctly', async () => {
            ...
            // Create test meals
            await setup_1.testPrisma.meal.createMany({
                data: [
                    {
                        userId,
    ...
    describe('sendDailySummary', () => {
        test('should send summary and log message on success', async () => {
            ...
            // Check message log
            const messageLog = await setup_1.testPrisma.messageLog.findFirst({
                where: { userId, direction: 'OUT' },
            });
            expect(messageLog).toBeTruthy();
            expect(messageLog.payload.type).toBe('daily_summary');
        });
...
//# sourceMappingURL=DailySummaryService.test.js.map